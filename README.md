...
Возьмем под хранилище фотографий 123 Тб/24=6 машин, а так же 3 машины, для обеспечения отказоустойчивости.

Для хранения таблиц, воспользуемся машинами с меньшим диском (1 Тб)

| CPU  (cores)  |      RAM (gb)    |  SSD (Tb) |
|:----------:|:-------------:|:------:|
| 32 |  64 | 1 |

Нашим основным приорететом является отказоустойчивость, так как каждая таблица с запасом вмещается на одну машину, на каждую таблицу выделим по машине с двумя слейвами, суммарно 9 машин

Так как основная нагрузка на сайт приходится на статический контент, и суммарный RPS на динамику составляет всего 6000  RPS, для бекендов закупим менее мощные машины: 
| CPU  (cores)  |      RAM (gb)    |  SSD (Gb) |
|:----------:|:-------------:|:------:|
| 16 |  32 | 256 | 

И разместим по машине с двумя слейвами на каждый микросервис для обеспечения отказоустойчивости, так что суммарно выйдет 9 машин для backend
